...
//   return (
// <div className=" w-full px-4 py-4 bg-[#F9FDFE]">
//   {/* "w-full px-4 py-4 bg-gray-100"> */}
...
import React, { useState } from "react";
import ShimmerLoader from "../components/ShimmerLoader"; // Reuse the existing ShimmerLoader

const Videos = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [videoUrl, setVideoUrl] = useState("");
  const [loaded, setLoaded] = useState(false); // Track whether the entire card is loaded
  ...
  // Handle when the entire card content is loaded
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {videos.map((video) => (
          <div
            ...
            onClick={() => openModal(video.youtubeLink)}
          >
            {/* Show shimmer loader until the content (image, title, description) is loaded */}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="relative w-11/12 sm:w-9/12 md:w-8/12 lg:w-1/2 p-4 rounded-lg">
            ...
            {/* Show shimmer loader until the video is loaded */}
            {!loaded && <ShimmerLoader />}
            <iframe
              className={`w-full h-[60vh] sm:h-[80vh] lg:h-[80vh] xl:h-[80vh] ${loaded ? "opacity-100" : "opacity-0"}`}
              src={videoUrl}
              title="YouTube Video"
              frameBorder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              onLoad={handleCardLoad} // Trigger handleCardLoad when iframe is ready
            ></iframe>
          </div>
        </div>
      )}
